...
class CriticNetwork(nn.Module):   
    def __init__(self, 
        ...
        self.linear1 = nn.Linear(num_filters*7*7+acts_dim+1, lin_1_dim) #todo: automatically calculate this
        ...
        self.compass_scaling = compass_scaling

    def forward(self, obs_pov, obs_rest, acts):
        
        x = self.conv1(obs_pov.permute(0,3,1,2) / self.pov_scaling)
        x = self.max_pool1(x)
        ...
        x = self.linear1(torch.cat([x, obs_rest / self.compass_scaling, acts], dim=1))
...
class ActorNetwork(nn.Module):
    def __init__(self, 
        ...
        # Fully connected layer
        self.linear1 = nn.Linear(num_filters*7*7+1, lin_1_dim) #todo: automatically calculate this
        self.linear2 = nn.Linear(lin_1_dim, lin_2_dim)
    ...
    def forward(self, obs_pov, obs_rest):
        ...
        x = x.view(self.non_lin_2.size(0), -1)
        x = self.linear1( torch.cat( [ x, obs_rest / self.compass_scaling] , dim=1 ) )
        self.non_lin_3 = F.relu(x)
        x = self.linear2(self.non_lin_3)
        self.non_lin_4 = F.relu(x)
...
class CriticNetworkSimp(nn.Module):   
    def __init__(self, 
                 acts_dim=1,
                 use_bn=False, 
                 compass_scaling=180,
                 lin_1_dim=128,
                 lin_2_dim=64):
        super(CriticNetworkSimp, self).__init__()

        # Fully connected layer
        self.linear1 = nn.Linear(acts_dim+1, lin_1_dim) #todo: automatically calculate this
        self.linear2 = nn.Linear(lin_1_dim, lin_2_dim)
        self.linear3 = nn.Linear(lin_2_dim, 1)
    ...
    def forward(self, obs_rest, acts):
        
       
        x = self.linear1(torch.cat([obs_rest / self.compass_scaling, acts], dim=1))
        self.non_lin_3 = F.relu(x)
        x = self.linear2(self.non_lin_3)
        self.non_lin_4 = F.relu(x)
        out = self.linear3(self.non_lin_4)
...
class ActorNetworkSimp(nn.Module):
    def __init__(self, 
                 acts_dim=1, 
                 use_bn=False, 
                 compass_scaling=180,
        ...
        super(ActorNetworkSimp, self).__init__()

        # Fully connected layer
        self.linear1 = nn.Linear(1, lin_1_dim) #todo: automatically calculate this
        self.linear2 = nn.Linear(lin_1_dim, lin_2_dim)
        self.mean_linear= nn.Linear(lin_2_dim, acts_dim)
        self.log_std_linear= nn.Linear(lin_2_dim, acts_dim)
    ...
    def forward(self, obs_rest):
        
        x = self.linear1(  obs_rest / self.compass_scaling  )
        self.non_lin_3 = F.relu(x)
        x = self.linear2(self.non_lin_3)
        self.non_lin_4 = F.relu(x)
    ...
    def get_action(self, obs_rest, deterministic=False):
        mean, log_std = self.forward(obs_rest)
        if deterministic:
            action = torch.tanh(mean)
        else:
